...
function MetodoCard({ icono, titulo, desc, badge, selected, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...card, padding: "20px", width: "100%", textAlign: "left", cursor: "pointer",
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
        <div style={{
          ...
          display: "flex", alignItems: "center", justifyContent: "center",
          color: selected ? C.accent : C.textSec, transition: "all 0.2s",
        }}>{icono}</div>
        <div style={{ flex: 1 }}>
...
export default function EntregasPage({ onVolver }) {
  ...
  const [paquete,      setPaquete]      = useState(null); // datos del inventario
  const [errorBusq,    setErrorBusq]    = useState("");
  const [escaneando,   setEscaneando]   = useState(false);

  // Paso 3 — precio (puede venir del inventario)
  ...
  const buscarGuia = async (codigoGuia) => {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:4000/api"}/inventario/guia/${codigoGuia.trim()}`,
        { headers: token ? { Authorization: `Bearer ${token}` } : {} }
      );
      if (!res.ok) throw new Error("Guía no encontrada en inventario");
  ...
  const simularEscaneo = () => {
    setEscaneando(true);
    setTimeout(() => {
      const codigoFake = `GUIA-${Math.floor(Math.random() * 90000) + 10000}`;
      setGuia(codigoFake);
      buscarGuia(codigoFake);
      setEscaneando(false);
    }, 2000);
  };
  ...
  const registrarEntrega = async () => {
    ...
    try {
      const token = getToken?.();
      await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:4000/api"}/entregas`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) },
          body: JSON.stringify({
            guia, precio: parseFloat(precio),
            metodo_pago: metodoPago, referencia,
            cantidad,
          }),
        }
      );
    } catch (e) { console.warn("Backend no disponible, simulando registro"); }
    setGuardando(false);
    setExito(true);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      ...
      <div style={{ ...card, flex: 1, display: "flex", flexDirection: "column", padding: "28px 32px", minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", gap: 24, maxWidth: 480, margin: "0 auto", width: "100%" }}>
              <div>
                <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 26, fontWeight: 900, color: C.textPrimary, margin: "0 0 6px" }}>
                  ¿Cuántas entregas deseas registrar?
                </h2>
          ...
          {/* ── PASO 2: Buscar guía en inventario ── */}
          {paso === 2 && (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 18, minHeight: 0, overflowY: "auto" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <MetodoCard
                  ...
                  desc="Escanea el código de barra de la guía para buscar automáticamente en el inventario."
                ...
                <MetodoCard
                  ...
                  desc="Ingresa manualmente el número de guía para buscarlo en el inventario."
              ...
              {/* Escanear */}
              {metodoInfo === "escanear" && (
                <div style={{ ...card, padding: "18px", background: "rgba(255,107,0,0.04)", border: `1px solid ${C.accentDim}`, display: "flex", flexDirection: "column", gap: 10 }}>
                  <button onClick={simularEscaneo} disabled={escaneando || buscando} style={{ ...btnOrange, opacity: (escaneando || buscando) ? 0.7 : 1 }}
                    onMouseEnter={e => { if (!escaneando && !buscando) e.currentTarget.style.background = "#E55E00"; }}
                    onMouseLeave={e => e.currentTarget.style.background = C.accent}>
                    {escaneando ? "Escaneando..." : buscando ? "Buscando en inventario..." : "Escanear Código"}
                  </button>
              ...
              {metodoInfo === "manual" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <input type="text" placeholder="Ej: GUIA-12345" value={guia} onChange={e => { setGuia(e.target.value); setPaquete(null); setErrorBusq(""); }}
                      style={{ ...inputStyle, flex: 1 }}
                      onFocus={e => e.target.style.borderColor = C.accent}
                      onBlur={e => e.target.style.borderColor = C.inputBorder} />
                    <button onClick={() => buscarGuia(guia)} disabled={!guia || buscando}
                      style={{ ...btnOrange, padding: "12px 20px", flexShrink: 0, opacity: (!guia || buscando) ? 0.5 : 1 }}
                      onMouseEnter={e => { if (guia && !buscando) e.currentTarget.style.background = "#E55E00"; }}
                      onMouseLeave={e => e.currentTarget.style.background = C.accent}>
                      {buscando ? "..." : "Buscar"}
                    </button>
                  </div>
              ...
              {paquete && (
                <div style={{ ...card, padding: "18px 20px", border: `1px solid rgba(16,185,129,0.25)`, background: C.successBg }}>
                  ...
                  <div style={{ marginTop: 10, padding: "8px 12px", borderRadius: 6, background: "rgba(16,185,129,0.08)", border: `1px solid rgba(16,185,129,0.2)` }}>
                    <p style={{ fontSize: 12, color: C.success, margin: 0 }}>
                      ✓ Al confirmar, el estado cambiará a <strong>Entregada</strong> y se enviará a Informes.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: "auto" }}>
                ...
                <button onClick={siguiente} disabled={!paquete} style={{ ...btnOrange, opacity: !paquete ? 0.4 : 1, cursor: !paquete ? "not-allowed" : "pointer" }}
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 20, minHeight: 0 }}>
              <div>
                ...
                <p style={{ fontSize: 13, color: C.textGhost, margin: 0 }}>
                  {precio ? `Precio del inventario: $${precio}. Puedes confirmarlo o ajustarlo.` : "Selecciona cómo ingresar el precio de esta entrega."}
                </p>
              </div>

              {precio && !metodoPrecio && (
                <div style={{ ...card, padding: "18px 20px", border: `1px solid ${C.accentDim}`, background: "rgba(255,107,0,0.04)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <MetodoCard
                  ...
                  titulo="Usar precio del inventario" badge="Recomendado"
                  desc={`Confirma el precio registrado: $${precio || "—"}`}
                  selected={metodoPrecio === "inventario"} onClick={() => setMetodoPrecio("inventario")}
                />
                <MetodoCard
                  icono={<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>}
                  titulo="Ajustar precio"
                  desc="Modifica el precio si hay un cobro diferente al registrado."
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    ...
                    <div onClick={() => fileRef.current?.click()} style={{
                      ...card, padding: "24px 20px", textAlign: "center", cursor: "pointer",
