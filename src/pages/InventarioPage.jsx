  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, flexShrink: 0, marginBottom: 16 }}>
        <StatCard
          icono={<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z"/></svg>}
          label="Entregas registradas" valor={totalEntregas} sub="Total de entregas" color={C.accent}
        />
        <StatCard
          icono={<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>}
          label="Envíos registrados" valor={totalEnvios} sub="Total de envíos" color="#6366f1"
        />
        <StatCard
          icono={<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>}
          label="Pendientes" valor={pendientes} sub="Por entregar" color={C.warning}
        />
        <StatCard
          icono={<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>}
          label="Entregados" valor={entregados} sub="Completados" color={C.success}
        />
      </div>
        ...
        <div style={{ display: "flex", borderBottom: `1px solid ${C.cardBorder}`, padding: "0 20px", flexShrink: 0 }}>
          {["entregas", "envios", "historial"].map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              ...
              color: tab === t ? C.accent : C.textGhost,
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0, marginBottom: 14, flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 200, position: "relative" }}>
                  <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: C.textGhost }}>
                    <IcoSearch />
                  </span>
                  <input
                    type="text" placeholder="Buscar por guía, cliente..."
                    value={filtroBusq} onChange={e => { setFiltroBusq(e.target.value); setPaginaE(1); setPaginaEnv(1); }}
                ...
                <button
                  onClick={() => setModal({ tipo: tab === "entregas" ? "entrega" : "envio" })}
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
                  <div style={{ flex: 1, overflow: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      <thead>
                        <tr>
                          {["Guía", "Cliente", "Valor", "Estado", "Fecha de registro", "Acciones"].map(h => (
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
                  <div style={{ flex: 1, overflow: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      <thead>
                        <tr>
                          {["Guía", "Destinatario", "Cliente", "Valor", "Método de pago", "Estado", "Fecha de registro", "Acciones"].map(h => (
                            <th key={h} style={thStyle}>{h}</th>
                          ))}
                        </tr>
                      </thead>
          ...
          {tab === "historial" && (
            <div style={{ flex: 1, overflow: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    {["Tipo", "Guía", "Cliente", "Valor", "Estado", "Método de pago", "Fecha"].map(h => (
                      <th key={h} style={thStyle}>{h}</th>
                    ))}
                  </tr>
                </thead>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 400, background: "rgba(0,0,0,0.65)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ ...card, maxWidth: 360, width: "100%", padding: "24px", animation: "fadeInUp 0.2s ease" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={() => eliminar(confirmDel.tipo, confirmDel.id)} style={{ flex: 1, padding: "10px", borderRadius: 8, border: "none", background: C.danger, color: "#fff", fontFamily: "'Barlow Condensed', sans-serif", fontSize: 14, fontWeight: 800, cursor: "pointer", letterSpacing: "0.05em", textTransform: "uppercase" }}
                onMouseEnter={e => e.currentTarget.style.background = "#dc2626"}
                onMouseLeave={e => e.currentTarget.style.background = C.danger}>Eliminar</button>
            </div>
          </div>
        </div>
      )}
