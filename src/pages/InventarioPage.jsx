...
function ModalForm({ tipo, item, inventario=[], onGuardar, onCerrar }) {
  ...
  const buscarEnInventario = useCallback((guia) => {
    ...
    setTimeout(() => {
      ...
      if (encontrado) { set("valor", encontrado.valor); setGuiaMsg(`✓ Precio del inventario: $${Number(encontrado.valor).toLocaleString()}`); }
      else setGuiaMsg("Guía no encontrada. Ingresa el precio manualmente.");
    ...
    <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(0,0,0,0.65)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ ...card, width:"100%", maxWidth:480, padding:"28px", animation:"fadeInUp 0.25s ease" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          {tipo==="entrega" ? (
            <div>
              ...
              <div style={{ position:"relative" }}>
                <input type="number" min="0" placeholder="0.00" value={form.valor}
          ...
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              {ESTADOS.map(e=>(
                <button key={e} onClick={()=>set("estado",e)} style={{ padding:"10px 12px", borderRadius:8, cursor:"pointer", border:`1.5px solid ${form.estado===e?(e==="Entregado"?C.success:C.danger):C.cardBorder}`, background:form.estado===e?(e==="Entregado"?C.successBg:C.dangerBg):"transparent", color:form.estado===e?(e==="Entregado"?C.success:C.danger):C.textGhost, fontFamily:"'Barlow',sans-serif", fontSize:13, fontWeight:600 }}>{e}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{ display:"flex", gap:10, marginTop:22 }}>
          <button onClick={onCerrar} style={{ flex:1, padding:"11px", borderRadius:8, border:`1px solid ${C.cardBorder}`, background:"transparent", color:C.textPrimary, fontFamily:"'Barlow',sans-serif", fontSize:14, fontWeight:600, cursor:"pointer" }}
            onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.05)"}
            onMouseLeave={e=>e.currentTarget.style.background="transparent"}>Cancelar</button>
          <button onClick={()=>onGuardar(form)} disabled={!form.guia||!form.cliente||!form.valor}
...
export default function InventarioPage({ onVolver }) {
  ...
  const eliminar = async (tipo, id) => {
    try {
      const token = getToken?.();
      await fetch(`${import.meta.env.VITE_API_URL||"http://localhost:4000/api"}/inventario/${tipo}s/${id}`,{method:"DELETE",headers:token?{Authorization:`Bearer ${token}`}:{}});
    } catch {}
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", minHeight:0, overflow:"hidden" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, flexShrink:0, marginBottom:16 }}>
        ...
        <StatCard icono={<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.7"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>} label="Entregados" valor={entregados} sub="Completados" color={C.success}/>
      </div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", minHeight:0, ...card, overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", borderBottom:`1px solid ${C.cardBorder}`, padding:"0 20px", flexShrink:0 }}>
          {["entregas","envios","historial"].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{ padding:"13px 16px", background:"none", border:"none", cursor:"pointer", fontSize:13, fontWeight:700, fontFamily:"'Barlow',sans-serif", color:tab===t?C.accent:C.textGhost, textTransform:"uppercase", letterSpacing:"0.06em", borderBottom:`2px solid ${tab===t?C.accent:"transparent"}`, marginBottom:-1 }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", minHeight:0, padding:"16px 20px", overflow:"hidden" }}>
            ...
            <>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, flexShrink:0, marginBottom:14, flexWrap:"wrap" }}>
                <div style={{ flex:1, minWidth:200, position:"relative" }}>
                  ...
                  <input type="text" placeholder="Buscar por guía, cliente..." value={filtroBusq}
                    onChange={e=>{ setFiltroBusq(e.target.value); setPaginaE(1); setPaginaEnv(1); }}
                    style={{ ...inputStyle, paddingLeft:36, fontSize:13 }}
                    onFocus={e=>e.target.style.borderColor=C.accent} onBlur={e=>e.target.style.borderColor=C.inputBorder}/>
                </div>
                <select value={filtroEstado} onChange={e=>{ setFiltroEstado(e.target.value); setPaginaE(1); setPaginaEnv(1); }}
                  style={{ ...inputStyle, width:"auto", minWidth:150, fontSize:13 }}
                  onFocus={e=>e.target.style.borderColor=C.accent} onBlur={e=>e.target.style.borderColor=C.inputBorder}>
                  <option value="Todos">Todos los estados</option>
                  {ESTADOS.map(e=><option key={e} value={e}>{e}</option>)}
                </select>
                <button onClick={()=>setModal({tipo:tab==="entregas"?"entrega":"envio"})}
                  style={{ ...btnOrange, whiteSpace:"nowrap" }}
                ...
                <div style={{ flex:1, display:"flex", flexDirection:"column", minHeight:0 }}>
                  <div style={{ flex:1, overflow:"auto" }}>
                    <table style={{ width:"100%", borderCollapse:"collapse" }}>
                      <thead><tr>{["Guía","Cliente","Valor","Estado","Fecha de registro","Acciones"].map(h=><th key={h} style={thStyle}>{h}</th>)}</tr></thead>
                      <tbody>
                        {sliceE.length===0
              ...
              {tab==="envios" && (
                <div style={{ flex:1, display:"flex", flexDirection:"column", minHeight:0 }}>
                  <div style={{ flex:1, overflow:"auto" }}>
                    <table style={{ width:"100%", borderCollapse:"collapse" }}>
                      <thead><tr>{["Guía","Destinatario","Cliente","Valor","Método de pago","Estado","Fecha","Acciones"].map(h=><th key={h} style={thStyle}>{h}</th>)}</tr></thead>
                      <tbody>
                        {sliceEnv.length===0
          ...
          {tab==="historial" && (
            <div style={{ flex:1, overflow:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead><tr>{["Tipo","Guía","Cliente","Valor","Estado","Método de pago","Fecha"].map(h=><th key={h} style={thStyle}>{h}</th>)}</tr></thead>
                <tbody>
                  {[...entregas.map(e=>({...e,tipo:"Entrega"})),...envios.map(e=>({...e,tipo:"Envío"}))]
                    ...
                    .map(row=>(
                      <tr key={`${row.tipo}-${row.id}`} onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.02)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                        <td style={tdStyle}>
                          <span style={{ fontSize:11, fontWeight:700, padding:"3px 8px", borderRadius:4, background:row.tipo==="Entrega"?C.accentDim:"rgba(99,102,241,0.18)", color:row.tipo==="Entrega"?C.accent:"#818cf8" }}>{row.tipo}</span>
                        </td>
                        <td style={{ ...tdStyle, color:C.accent, fontWeight:700 }}>{row.guia}</td>
                        <td style={tdStyle}>{row.cliente}</td>
                        ...
                        <td style={tdStyle}>
                          {row.metodo_pago
                            ? <span style={{ display:"inline-flex", alignItems:"center", gap:5 }}><span style={{ width:6, height:6, borderRadius:"50%", background:row.metodo_pago==="Efectivo"?C.success:"#6366f1" }}/>{row.metodo_pago}</span>
      ...
      {/* Modal formulario */}
      {modal && <ModalForm tipo={modal.tipo} item={modal.item||null} inventario={entregas} onGuardar={guardar} onCerrar={()=>setModal(null)}/>}
        ...
        <div style={{ position:"fixed", inset:0, zIndex:400, background:"rgba(0,0,0,0.65)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
          <div style={{ ...card, maxWidth:360, width:"100%", padding:"24px", animation:"fadeInUp 0.2s ease" }}>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={()=>setConfirmDel(null)} style={{ flex:1, padding:"10px", borderRadius:8, border:`1px solid ${C.cardBorder}`, background:"transparent", color:C.textPrimary, fontFamily:"'Barlow',sans-serif", fontSize:14, fontWeight:600, cursor:"pointer" }}
                onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.05)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>Cancelar</button>
              <button onClick={()=>eliminar(confirmDel.tipo,confirmDel.id)} style={{ flex:1, padding:"10px", borderRadius:8, border:"none", background:C.danger, color:"#fff", fontFamily:"'Barlow Condensed',sans-serif", fontSize:14, fontWeight:800, cursor:"pointer", textTransform:"uppercase" }}
      ...
      <style>{`@keyframes fadeInUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}} select option{background:#161616;color:#f0f4f8;}`}</style>
    </div>
  );
}