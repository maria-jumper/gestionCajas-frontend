  ...
  return jsonRows
    ...
    .map((row, i) => {
      ...
      return {
        ...
        estado:       "No entregado", // Siempre No entregado al importar — se actualiza al registrar entrega
        fecha:        normalizarFecha(norm.fecha),
...
// Convierte cualquier formato de fecha a YYYY-MM-DD para MySQL
...
function ModalDetalle({ item, onCerrar }) {
  ...
  const campos = [
    ...
    { l:"Fecha registro", v:fmtFecha(item.fecha) },
              ...
              <tbody>
                {slice.map(row => (
                  <tr key={row.id} style={{ transition:"background 0.12s" }}
                    ...
                    <td style={tdStyle}>{fmtFecha(row.fecha)}</td>
                    <td style={tdStyle}>
